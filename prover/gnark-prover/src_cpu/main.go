...
// DeviceInfo represents device information for CPU mode
type DeviceInfo struct {
	TaskCount int32
}

// server implements the gRPC proving service
type server struct {
	__.UnimplementedProveServiceServer
...
// loadDevice initializes device information for CPU-only mode
func loadDevice() (map[int]*DeviceInfo, error) {
	devices := make(map[int]*DeviceInfo)
	log.Printf("[loadDevice] Initializing CPU-only mode")

	// Create a virtual device for CPU mode
	devices[0] = &DeviceInfo{
		TaskCount: 0,
	}

	log.Printf("[loadDevice] Successfully loaded CPU-only mode")
	return devices, nil
}
...
// getDevice returns the least loaded device for CPU mode
func (s *server) getDevice() (interface{}, int) {
	selectedIndex := -1
	minTasks := int32(1<<31 - 1)

	for i, devInfo := range s.devices {
		taskCount := atomic.LoadInt32(&devInfo.TaskCount)
	...
	return nil, selectedIndex // CPU mode - no actual device
}

// ZbFrCircuit represents the Binance futures circuit
...
// TigaCircuit represents the Tiga blockchain verification circuit
type TigaCircuit struct {
	...
	// Project identifier (public)
	ProjectId frontend.Variable `gnark:",public"`
}

// FancasterAnonymousCircuit represents the Fancaster anonymous transaction circuit
...
// Define implements the circuit constraints for TigaCircuit
...
// load reads and parses the Binance data file
...
// Define implements the circuit constraints for ZbFrCircuit
func (circuit *ZbFrCircuit) Define(api frontend.API) error {
	return nil
}

// ProveBinanceOffchain generates proof for Binance off-chain data
func (s *server) ProveBinanceOffchain(ctx context.Context, in *__.ProveRequest) (*__.ProveResponseV2, error) {
	totalStart := time.Now()
	circuitId := in.GetTemp()
	circuitData, ok := s.circuits[circuitId]
	if !ok {
		return &__.ProveResponseV2{
	...
	if len(in.GetInput()) > 1 {
		...
		asset := in.GetInput()
		// Map asset symbols to futures symbols
		assetCoinSymbol := map[string]string{
			"BTC": "BTCUSD_PERP",
			"ETH": "ETHUSD_PERP",
		}
		...
		for _, v := range data.FuturesCoinPositions {
			if v.Symbol == assetCoinSymbol[asset] {
				vleverage, err := strconv.ParseInt(v.Leverage, 10, 64) // Base 10, 64-bit width
				if err != nil {
					log.Printf("Error parsing leverage: %v", err)
					continue
				}
				...
				// Convert to satoshis
				value := v.NotionalValue * SatoshiMultiplier
				if value > 0 {
					perpLong += value
				} else {
					perpShort += value
		...
		for _, v := range data.FuturesCoinAssets {
			if v.Symbol == asset {
				// Convert to satoshis
				walletBalance += v.WalletBalance * SatoshiMultiplier
				marginBalance += v.MarginBalance * SatoshiMultiplier
			}
		}
		...
		rawProof, err := groth16.Prove(circuitData.Ccs, circuitData.Pk, secretWitness)
		if err != nil {
			return &__.ProveResponseV2{
...
// ProveTigaOffchain generates proof for Tiga blockchain data
func (s *server) ProveTigaOffchain(ctx context.Context, in *__.ProveRequest) (*__.ProveResponseV2, error) {
	totalStart := time.Now()
	circuitId := in.GetTemp()
	circuitData, ok := s.circuits[circuitId]
	if !ok {
...
// Define implements the circuit constraints for FancasterAnonymousCircuit
func (circuit *FancasterAnonymousCircuit) Define(api frontend.API) error {
	return nil
}

// ProveMerkleOffchain generates proof for Merkle tree verification
func (s *server) ProveMerkleOffchain(ctx context.Context, in *__.ProveRequest) (*__.ProveResponseV2, error) {
	totalStart := time.Now()
	circuitId := in.GetTemp()
	circuitData, ok := s.circuits[circuitId]
	if !ok {
...
// GetRunningProveTasks returns the current number of running prove tasks
func (s *server) GetRunningProveTasks(ctx context.Context, in *__.Empty) (*__.GetRunningProveTasksResponse, error) {
	count := atomic.LoadInt32(&s.runningProveTasks)
	return &__.GetRunningProveTasksResponse{
		Code:  0,
		Msg:   "Success",
		Count: count,
	}, nil
...
func (s *server) Ping(ctx context.Context, in *__.Empty) (*__.Empty, error) {
...
func main() {
	...
	log.Printf("Starting Gnark Prover (CPU mode), version: %s", Version)

	// Load circuit templates
	loadCircuitsStart := time.Now()
	circuits, err := loadAllCircuits(*templateFolderPath)
	if err != nil {
		log.Fatalf("Failed to load circuits: %v", err)
	}
	log.Printf("Loaded %d circuits in %v", len(circuits), time.Since(loadCircuitsStart))

	// Initialize device for CPU mode
	devices, err := loadDevice()
	if err != nil {
