...
class Config:
    ...
    def _load_config(self):
        mode = os.environ.get('MODE', 'NODE').lower()
        try:
            custom_config_module = importlib.import_module(f'.{mode}', package=__package__)
            self._config: NodeConfig = custom_config_module.Config
            logging.info(f"[Config] - [{mode}] is loaded")
        except ImportError:
            logging.error(f"[Config] - [{mode}] cannot be imported, falling back to [NODE]")
