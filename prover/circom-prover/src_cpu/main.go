...
// ProveV2 generates proof with enhanced output format
...
// ProveNosha256 generates proof without SHA256 verification
...
// ProveNosha256Offchain generates off-chain proof without SHA256 verification
func (s *server) ProveNosha256Offchain(ctx context.Context, in *pb.ProveNosha256Request) (*pb.ProveNosha256OffchainResponse, error) {
	...
	if len(in.GetInput()) > 1 {
		...
		proof, err := groth16.Prove(circuitData.Ccs, circuitData.Pk, secretWitness)
		if err != nil {
			return &pb.ProveNosha256OffchainResponse{
				Code: STATUS_CODE_GENERATE_PROOF_ERROR,
				Msg:  "Error Prove",
...
// ProveNosha256WithWitness generates proof with witness data
...
// Ping returns server health status and basic information
...
func main() {
	...
	log.Printf("Starting Circom Prover (CPU mode), version: %s", Version)
