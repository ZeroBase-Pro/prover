...
func ReadR1CS(filename string) (constraint.ConstraintSystem, uint32, uint32, error) {
	...
	// log raw header bytes for debug purposes
	lg := logger.Logger()
	lg.Debug().Msgf("raw header bytes: %d %d %d %d", file[ptr], file[ptr+1], file[ptr+2], file[ptr+3])
	...
	lg = logger.Logger()
	lg.Info().Msgf("Header info: nVars=%d nOutputs=%d nPubInputs=%d nPriInputs=%d nLabels=%d nConstraints=%d", nVars, nOutputs, nPubIntputs, nPriInputs, nLabels, nConstraints)
	...
	lg.Info().Msgf("Read time: %v", durationRead)
	...
	lg.Info().Msgf("Compile time: %v", duration)
