...
case "$MODE" in
  gpu)
    IMAGE="${IMAGE:-zer0base/circom-prover:gpu-${VER}}"
    NAME="${NAME:-circom-prover}"
    ;;
  cpu)
    IMAGE="${IMAGE:-zer0base/circom-prover:cpu-${VER}}"
    NAME="${NAME:-circom-prover}"
    ;;
  *)
...
if [[ "$MODE" == "gpu" ]]; then
  docker run -d --name "${NAME}" \
    ...
    -e REQUIRE_TLS="${REQUIRE_TLS:-true}" \
    -e SSL_CERTFILE="${SSL_CERTFILE:-/app/certs/tls.crt}" \
    -e SSL_KEYFILE="${SSL_KEYFILE:-/app/certs/tls.key}" \
